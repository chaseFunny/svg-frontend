...
export function downloadSvgAsPng(svgContent: string, filename = `svg-${Date.now()}.png`, scale = 4) {
  ...
  // 如果viewBox未获取到有效尺寸，尝试从width/height属性获取
  ...
  if (!svgWidth || !svgHeight) {
    svgWidth = 300;
    svgHeight = 150;
    console.warn("无法从SVG获取尺寸，使用默认值300x150");
  }
  ...
  if (!ctx) {
    toast.error("浏览器不支持Canvas，无法导出PNG");
    return;
  }
